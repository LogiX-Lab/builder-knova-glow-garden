  ...
  const items = [
    {
      ...
      title: "Great Teacher",
      desc: "Uniquely productivate real time collaboration idea-sharing after awesome quality vectors after.",
    },
    {
      color: "bg-pink-500",
        ...
        {items.map((it) => (
          <div key={it.title} className="relative rounded-2xl">
            <div className="mx-auto flex max-w-[1170px] flex-col items-center gap-4 text-center">
              <div
                className={`flex h-24 w-24 items-center justify-center rounded-full ${it.color} p-5`}
              >
                <img
                  src={it.icon}
                  alt=""
